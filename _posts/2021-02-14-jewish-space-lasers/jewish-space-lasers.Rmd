...
author:
  - name: Jonathan M. Davis
    ...
    url: {}
  - name: Joshua Cherof
    affiliation: Jewish Subject Matter Expert
date: 02-15-2021
output:
...
## System Requirements
The authors conducted user base surveys to elicit the needs of the Jewish Space Laser enthusiast community using a modified Delphi panel method ^[JMD asked JC what he wanted the laser system to do in an exchange of facebook comments]. The modifications to the panel method reduced cost and facilitated rapid requirements development. For this analysis the laser system under examination must:
...
```{r PowerOpt, echo=F, fig.cap="Some incomprehensible charts from:https://apps.dtic.mil/dtic/tr/fulltext/u2/781763.pdf", out.extra="class=external"}
...
```{r result, fig.height=4.5,fig.width=7.5, fig.cap="Sensitivity of system power requirements to laser characteristics", echo=FALSE }


ggplot(subset(LaserParam, M2<=20) , aes(x = Aperture, y = GWatts, colour = M2, group = M2)) + geom_line() + theme_minimal()

```

Solaren's planned solar power generation plant will generate 200 MW by design. Based on our assumptions regarding feasible apertures,  propagation ratios, wall plug efficiency, and the effect of the atmosphere, the minimal power plant capacity required to support this system is `r round(min(LaserParam$GWatts), 1)` GW. Even if the atmosphere has no effect on the beam energy the system power required is still `r round(min(LaserParam$MWatts)/10, 0)` MW, far exceeding Solaren's proposed design. 

We encourage the Jewish Space Laser enthusiast community to use these results to inform future system designs and technology investments. Obviously developing large aperture, high quality beams is of paramount importance to deploying such a system. While staging a pyrotechnic gender reveal party or having an agent simply toss a lighter is astronomically cheaper, such crude tactics lack a sense of sophistication and are unbecoming of Jewish Space Laser enthusiasts. 
